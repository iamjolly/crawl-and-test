...
function generateHTMLReport(data, filename) {
  ...
  if (Array.isArray(data)) {
    ...
    const pagesWithIssues = pages.filter(
      page => (page.violations?.length || 0) > 0 || (page.incomplete?.length || 0) > 0
    ).length;
  ...
  const summaryCards = [
    {
      title: '<span aria-hidden="true">📄</span> Total Pages',
      value: summary.totalPages || 0,
      type: 'info',
    },
    {
      title: '<span aria-hidden="true">❌</span> Total Violations',
      value: summary.totalViolations || 0,
      type: 'error',
    },
    {
      title: '<span aria-hidden="true">⚠️</span> Total Warnings',
      value: summary.totalIncomplete || 0,
      type: 'warning',
    },
    {
      title: '<span aria-hidden="true">✅</span> Pages Passed',
      value: summary.pagesPassed || 0,
      type: 'success',
    },
  ]
    .map(card =>
    ...
    const pageCards = pages
      .map((page, index) => {
        ...
        if (violations.length > 0) {
          pageStats.push(
            `<span class="stat-badge stat-violations"><span aria-hidden="true">❌</span> ${violations.length} violation${violations.length !== 1 ? 's' : ''}</span>`
          );
        }
        if (incomplete.length > 0) {
          pageStats.push(
            `<span class="stat-badge stat-warnings"><span aria-hidden="true">⚠️</span> ${incomplete.length} warning${incomplete.length !== 1 ? 's' : ''}</span>`
          );
        }
        if (totalIssues === 0) {
          pageStats.push(
            '<span class="stat-badge stat-passed"><span aria-hidden="true">✅</span> No issues</span>'
          );
        }
        ...
        if (totalIssues === 0) {
          ...
          // Process violations
          if (violations.length > 0) {
            issuesContent += generateIssuesSection(
              '<span aria-hidden="true">❌</span> Violations',
              violations,
              'violation'
            );
          }
        ...
        return pageCardTemplate
          ...
          .replace(
            /{{pageTitle}}/g,
            page.pageTitle ? `${index + 1}. ${page.pageTitle}` : `Page ${index + 1}`
          )
  ...
  if (domainFromFilename && wcagVersionFromFilename && wcagLevelFromFilename) {
    ...
    try {
      ...
      if (fs.existsSync(domainDir)) {
        ...
        const matchingJson = files.find(
          file =>
            file.startsWith(
              `${domainFromFilename}_wcag${wcagVersionFromFilename}_${wcagLevelFromFilename}_`
            ) && file.endsWith('.json')
        );
        if (matchingJson) {
  ...
  function scanDirectory(dir) {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (targetDomain && item !== targetDomain) {
          continue;
        }
