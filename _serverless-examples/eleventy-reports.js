...
module.exports = async function () {
  ...
  try {
    ...
    for (const prefix of response.CommonPrefixes || []) {
      ...
      const htmlReports = domainReports.Contents.filter(obj => obj.Key.endsWith('.html'))
        .map(obj => ({
          domain,
    ...
    return {
      reports: reports.sort(
        (a, b) => new Date(b.latestReport.lastModified) - new Date(a.latestReport.lastModified)
      ),
